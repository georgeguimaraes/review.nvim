...
T["comments survive buffer recreation"] = MiniTest.new_set()

T["comments survive buffer recreation"]["re-renders comments on a new buffer for the same file"] = function()
  ...
  eq(2, #new_extmarks_after)

  vim.api.nvim_buf_delete(new_buf, { force = true })
end

T["comments survive buffer recreation"]["re-renders file comments on a new buffer"] = function()
  local ns_id = vim.api.nvim_create_namespace("review")
  ...
  eq(1, #new_extmarks)
  eq(true, new_extmarks[1][4].virt_lines_above)

  vim.api.nvim_buf_delete(new_buf, { force = true })
end

T["comments survive buffer recreation"]["re-renders alignment padding on new buffers"] = function()
...
T["CodeDiffOpen autocmd"] = MiniTest.new_set()

T["CodeDiffOpen autocmd"]["autocmd is registered after setup"] = function()
